...
- **Messaging metadata, impersonation, and coordination tampering.** Private messages ride the xx network mixnet end-to-end encrypted, so a network observer sees neither their content nor who is talking to whom (mixnet metadata-resistance is the whole point). A contact is an account-signed binding: the wallet verifies the account key really signed that cMix identity before storing it, and only auto-accepts a channel request whose canonical reception ID matches a known contact — so a contact can't be faked or spoofed. Each of your accounts uses a *separate* cMix identity, so your accounts can't be linked to one another through messaging. A multisig proposal delivered over the mixnet is TRANSPORT only — the received bytes-package is re-validated against the on-chain call hash before anything is surfaced for approval, exactly like a pasted or QR'd one, so a malicious sender cannot smuggle a different call in under a friendly description.
...
- **The messaging identity on a compromised device.** Messaging is opt-in and default-off. Each account's cMix identity is protected at rest by a *dedicated messaging passphrase* — separate from any wallet-account password, scrypt-wrapped — and is an access credential for a comms identity, never a fund-signing factor. But like any secret in a browser wallet, a compromised device or browser, or active same-origin script, can read it while the app is unlocked; and an exported backup is only as safe as the passphrase guarding it (a weak passphrase is crackable offline against the KDF). Messaging does not defend against a global passive adversary capable of defeating the mixnet itself — that is xx network's own threat model — nor against a contact you deliberately added and chose to trust.
...
1. The GitHub repository — code that lands on `main` deploys automatically. `main` is branch-protected: it rejects direct pushes for all users including the repository owner, and accepts only pull requests from the `beta` integration branch with the CI status check green (typecheck, full test suite, an architectural boundary check, and a full-history secret scan). This narrows the attack from "any push" to "a merged pull request," though an attacker with full account control could still alter the protection rules — repository-account compromise remains in the trust base.
